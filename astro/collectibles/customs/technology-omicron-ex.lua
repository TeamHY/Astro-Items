...
if EID then
	Astro:AddEIDCollectible(
		...
		"테크놀로지 오미크론 EX",
		"진정한 레이저 고리 눈물",
		"↓ {{TearsSmall}}연사 x0.8#공격 시 50%의 확률로 눈물 주변에 레이저가 생깁니다.#중첩 시 기본 확률이 합 연산으로 증가하고 쿨타임이 줄어들며, 레이저 고리가 추가됩니다.#소환 쿨타임 : 0.1초#!!! {{LuckSmall}}행운 수치 비례: 행운 10 이상일 때 100% 확률 (행운 1당 +5%p)"
	)
end
...
local function ShouldTearGetEffects(tear)
	...
	return (WasTearFiredByPlayerMimic(tear) and priority < tearVariantPriority.TEAR_EFFECTS_THRESHOLD and
	...
	function(_, tear)
		...
		if not data.calledRetributionFireTearCallbacks then
			if tear.FrameCount < 1 and tear.Parent and not tear:GetData().dowsing then
				if
					tear.Parent.Type == 1 or
						(tear.Parent.Type == 3 and (tear.Parent.Variant == 80 or tear.Parent.Variant == 235 or tear.Parent.Variant == 240))
				 then
...
Astro:AddCallback(
	...
	function(_, knife)
		if knife.SpawnerEntity and knife.SpawnerEntity:ToPlayer() then
			...
			if isFlying then
				local isReturning =
					data.retributionLastFrameDistance and
