...
function milkshake:onCache(player, cacheFlag)
    if not player:HasCollectible(Astro.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS) then return end

    local rng = isc:copyRNG(player:GetCollectibleRNG(Astro.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS))
    local itemNum = player:GetCollectibleNum(Astro.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS)
...
function milkshake:OnMilkshakeAdded(player, _, firstTime)
    if IsPlayerShowingStatsUI(player) then
        ...
        if not Astro.Data.AcsogMultiplierFramePerPlayer then
            Astro.Data.AcsogMultiplierFramePerPlayer = {}
        end

        local multiplierCounterFramesPerPlayer = Astro.Data.AcsogMultiplierFramePerPlayer
...
local function RenderMultiplier(player, startingFrame)
    ...
    if not player:HasCollectible(Astro.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS) then return true end
