...
if EID then
    Astro:AddEIDCollectible(
        ...
        "하트 꽃",
        "식물 대 아이작",
        "30%의 확률로 적에게 달라붙는 씨앗 공격이 나갑니다." ..
        "#{{ArrowGrayRight}} 적에게 달라붙은 씨앗은 2초 후 공격력 x6 +35의 폭발 피해를 줍니다." ..
        "#{{Blank}} {{ColorGray}}(캐릭터는 피해를 입지 않음){{CR}}" ..
...
local function OnTearInit(_, tear)
    ...
    if not player:HasCollectible(Astro.Collectible.HEART_BLOOM) then return end

    local rng = player:GetCollectibleRNG(Astro.Collectible.HEART_BLOOM)
