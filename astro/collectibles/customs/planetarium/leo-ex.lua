...
Astro:AddCallback(
    ...
    function(_, player, collectibleType)
        ...
        itemPool:RemoveCollectible(CollectibleType.COLLECTIBLE_LEO)

        if not hiddenItemManager:Has(player, CollectibleType.COLLECTIBLE_LEO) then
...
Astro:AddCallback(
    ...
    ---@param collectibleType CollectibleType
    function(_, player, collectibleType)
        if hiddenItemManager:Has(player, CollectibleType.COLLECTIBLE_LEO) then
            hiddenItemManager:Remove(player, CollectibleType.COLLECTIBLE_LEO)
        end
    end,
    Astro.Collectible.LEO_EX
)
        ...
        for i = 1, room:GetGridSize() do
            ...
            if grid and grid:ToRock() then
                ...
                if prevState ~= 2 and currentState == 2 then
                    ...
                    if pData["LeoEXDestroyCount"] % (ROCK_COUNT - leoNum + 1) == 0 then
