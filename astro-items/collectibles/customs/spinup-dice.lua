...
AstroItems.Collectible.SPINUP_DICE = Isaac.GetItemIdByName("Spinup Dice")
...
if EID then
    AstroItems:AddEIDCollectible(
        ...
        "스핀업 주사위",
        "+1",
        "사용 시 방 안의 모든 아이템을 코드 앞번호의 아이템으로 바꿉니다.#!!! 일부 아이템은 등장하지 않습니다."
    )
end
...
local function TabConditions(descObj)
    if descObj.ObjType == 5 and descObj.ObjVariant == 100 and EID:PlayersActionPressed(EID.Config["BagOfCraftingToggleKey"]) and not EID.inModifierPreview then
        ...
        for i = 0, numPlayers - 1 do
            if Isaac.GetPlayer(i):HasCollectible(AstroItems.Collectible.SPINUP_DICE) or (EID.absorbedItems[tostring(i)] and EID.absorbedItems[tostring(i)][tostring(AstroItems.Collectible.SPINUP_DICE)]) then
                return true
            end
        end
    end
...
local function SpinupModifierCallback(descObj)
    ...
    for i = 0, numPlayers - 1 do
        if Isaac.GetPlayer(i):HasCollectible(AstroItems.Collectible.SPINUP_DICE) or (EID.absorbedItems[tostring(i)] and EID.absorbedItems[tostring(i)][tostring(AstroItems.Collectible.SPINUP_DICE)]) then
            playerID = i
            icon = "#{{Collectible" .. AstroItems.Collectible.SPINUP_DICE .. "}} :"
            break
        end
    end
    if playerID then
...
local function GetAvailableSkipAmount()
    ...
    for i = 0, playerCount - 1 do
        ...
        if p ~= nil and ( p:GetActiveItem ( ActiveSlot.SLOT_PRIMARY ) == AstroItems.Collectible.SPINUP_DICE or p:GetActiveItem ( ActiveSlot.SLOT_POCKET ) == AstroItems.Collectible.SPINUP_DICE ) then
            if hasCarBattery then
                spinAmount = spinAmount + 1
            end
