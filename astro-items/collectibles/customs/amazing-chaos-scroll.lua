...
AstroItems.Collectible.AMAZING_CHAOS_SCROLL = Isaac.GetItemIdByName("Amazing Chaos Scroll")
...
AstroItems:AddCallback(
    ...
    function(_, isContinued)
        if not isContinued then
            AstroItems.Data.MilkshakeMultiplierFramePerPlayer = {}
        end
    end
)
...
function milkshake:onCache(player, cacheFlag)
    if not player:HasCollectible(AstroItems.Collectible.AMAZING_CHAOS_SCROLL) then return end

    local rng = isc:copyRNG(player:GetCollectibleRNG(AstroItems.Collectible.AMAZING_CHAOS_SCROLL))
    local itemNum = player:GetCollectibleNum(AstroItems.Collectible.AMAZING_CHAOS_SCROLL)
...
function milkshake:OnMilkshakeAdded(player, _, firstTime)
    if IsPlayerShowingStatsUI(player) then
        ...
        if not AstroItems.Data.MilkshakeMultiplierFramePerPlayer then
            AstroItems.Data.MilkshakeMultiplierFramePerPlayer = {}
        end

        local multiplierCounterFramesPerPlayer = AstroItems.Data.MilkshakeMultiplierFramePerPlayer
...
local function RenderMultiplier(player, startingFrame)
    ...
    if not player:HasCollectible(AstroItems.Collectible.AMAZING_CHAOS_SCROLL) then return true end
