AstroItems.Collectible.BACHELORS_DEGREE = Isaac.GetItemIdByName("Bachelor's Degree")

if EID then
    AstroItems:AddEIDCollectible(AstroItems.Collectible.BACHELORS_DEGREE, "학사학위", "...", "{{Pill1}}Gulp!와 스탯 관련 알약만 등장합니다.#긍정 및 부정 효과가 전부 무시됩니다")
end
...
AstroItems:AddCallback(
    ...
    function(_, selectedPillEffect, pillColor)
        if not AstroItems.Data.Pills then
            return selectedPillEffect
        end

        if AstroItems.Data.Pills[pillColor] then
            return AstroItems.Data.Pills[pillColor]
        end

        for i = 1, Game():GetNumPlayers() do
            local player = Isaac.GetPlayer(i - 1)

            if player:HasCollectible(AstroItems.Collectible.BACHELORS_DEGREE) then
                ...
                if CheckWhitelist(selectedPillEffect, whiteList) then
                    AstroItems.Data.Pills[pillColor] = selectedPillEffect
                else
                    local rng = player:GetCollectibleRNG(AstroItems.Collectible.BACHELORS_DEGREE)
                    local index = rng:RandomInt(#whiteList) + 1
                    local newPillEffect = whiteList[index]
    
                    AstroItems.Data.Pills[pillColor] = newPillEffect
                end
