...
AstroItems.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS = Isaac.GetItemIdByName("Amazing Chaos Scroll of Goodness")

if EID then
    AstroItems:AddEIDCollectible(
        AstroItems.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS,
        "놀라운 긍정의 혼돈의 주문서",
        "정상화",
        "{{UnknownHeart}} 최대 체력, 소울하트, 블랙하트 중 하나 +1" ..
        "#{{ArrowUp}} {{SpeedSmall}}이동속도 배율 x1.0~1.35" ..
        "#{{ArrowUp}} {{TearsSmall}}공격 딜레이 ÷1.0~1.35" ..
        "#{{ArrowUp}} {{DamageSmall}}공격력 배율 x1.0~1.35" ..
        "#{{ArrowUp}} {{RangeSmall}}사거리 배율 x1.0~1.35" ..
        "#{{ArrowUp}} {{ShotspeedSmall}}탄속 배율 x1.0~1.35" ..
        "#{{ArrowUp}} {{LuckSmall}}행운 배율 x1.0~1.35" ..
...
AstroItems:AddCallback(
    ...
    function(_, isContinued)
        if not isContinued then
            AstroItems.Data.AcsogMultiplierFramePerPlayer = {}
        end
    end
)
...
function milkshake:onCache(player, cacheFlag)
    if not player:HasCollectible(AstroItems.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS) then return end

    local rng = isc:copyRNG(player:GetCollectibleRNG(AstroItems.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS))
    local itemNum = player:GetCollectibleNum(AstroItems.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS)
...
function milkshake:OnMilkshakeAdded(player, _, firstTime)
    if IsPlayerShowingStatsUI(player) then
        ...
        if not AstroItems.Data.AcsogMultiplierFramePerPlayer then
            AstroItems.Data.AcsogMultiplierFramePerPlayer = {}
        end

        local multiplierCounterFramesPerPlayer = AstroItems.Data.AcsogMultiplierFramePerPlayer
...
local function RenderMultiplier(player, startingFrame)
    ...
    if not player:HasCollectible(AstroItems.Collectible.AMAZING_CHAOS_SCROLL_OF_GOODNESS) then return true end
