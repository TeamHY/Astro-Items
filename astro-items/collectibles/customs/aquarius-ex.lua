AstroItems.Collectible.AQUARIUS_EX = Isaac.GetItemIdByName("Aquarius EX")

if EID then
    AstroItems:AddEIDCollectible(AstroItems.Collectible.AQUARIUS_EX, "초 물병자리", "...", "공격 당한 몬스터에서 눈물이 뿜어져 나옵니다.")
end
...
AstroItems:AddCallback(
    ...
    function(_, entity, amount, damageFlags, source, countdownFrames)
        ...
        if player ~= nil and player:HasCollectible(AstroItems.Collectible.AQUARIUS_EX) and entity:IsVulnerableEnemy() then
            if
             ...
             then
                entity:GetData().Aquarius = {
                    Source = player,
                    Delay = math.floor(7 / player:GetCollectibleNum(AstroItems.Collectible.AQUARIUS_EX)) --몬스터에게 나오는 눈물 설정하는 곳
                }
            end
        end
    end
