...
AstroItems.Collectible.DEATHS_EYES = Isaac.GetItemIdByName("Death's Eyes")
...
AstroItems:AddCallback(
    ...
    function(_, tear)
        ...
        if player ~= nil then
            if
                player:HasCollectible(AstroItems.Collectible.DEATHS_EYES) and
                    not player:HasCollectible(CollectibleType.COLLECTIBLE_GODHEAD)
             then
                local rng = player:GetCollectibleRNG(AstroItems.Collectible.DEATHS_EYES)

                -- 눈물에 후광을 직접 적용하면 이펙트 오류가 발생한다. 따라서 리페어 모드와 동일한 아래 코드를 사용한다.
                player.TearFlags = player.TearFlags & ~TearFlags.TEAR_GLOW

                if rng:RandomFloat() < 0.25 * player:GetCollectibleNum(AstroItems.Collectible.DEATHS_EYES) then
                    player.TearFlags = player.TearFlags | TearFlags.TEAR_GLOW
                end
