...
AstroItems.Collectible.SANDEVISTAN = Isaac.GetItemIdByName("Sandevistan")
AstroItems:AddSoulItem(AstroItems.Collectible.SANDEVISTAN)

if EID then
    AstroItems:AddEIDCollectible(
        AstroItems.Collectible.SANDEVISTAN,
        "산데비스탄",
        "...",
...
AstroItems:AddCallback(
    ...
    function(_, isContinued)
        if not isContinued then
            AstroItems.Data["sandevistanSouls"] = 0
        end
    end
)

AstroItems:AddCallback(
    AstroItems.Callbacks.SOUL_COLLECTED,
    ---@param player EntityPlayer
    function(_, player)
        local data = AstroItems.Data

        AstroItems.Data["sandevistanSouls"] = AstroItems.Data["sandevistanSouls"] + 1

        if AstroItems.Data["sandevistanSouls"] > maxinum then
            AstroItems.Data["sandevistanSouls"] = maxinum
        end
    end
)
...
AstroItems:AddCallback(
    ...
    function(_, item, rng, player, useFlags, activeSlot, varData)
        ...
        if not AstroItems.Data["sandevistanSouls"] or AstroItems.Data["sandevistanSouls"] < lowPoint then
            discharge = false
            return {
                Discharge = discharge,
                Remove = remove,
     ...
     if player:GetEffects():HasCollectibleEffect(CollectibleType.COLLECTIBLE_ASTRAL_PROJECTION) then
        ...
        if AstroItems.Data["sandevistanSouls"] >= highPoint then
            ...
            AstroItems.Data["sandevistanSouls"] = AstroItems.Data["sandevistanSouls"] - highPoint
        elseif AstroItems.Data["sandevistanSouls"] >= middlePoint then
            player:GetEffects():AddNullEffect(NullItemID.ID_CAMO_BOOST, false, 30)
            AstroItems.Data["sandevistanSouls"] = AstroItems.Data["sandevistanSouls"] - middlePoint
        elseif AstroItems.Data["sandevistanSouls"] >= lowPoint then
            AstroItems.Data["sandevistanSouls"] = AstroItems.Data["sandevistanSouls"] - lowPoint
        end
...
AstroItems:AddCallback(
    ...
    function(_)
        for i = 1, Game():GetNumPlayers() do
            ...
            if player:HasCollectible(AstroItems.Collectible.SANDEVISTAN) then
                local souls = AstroItems.Data["sandevistanSouls"] or 0
