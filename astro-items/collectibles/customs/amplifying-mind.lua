...
if EID then
    AstroItems:AddEIDCollectible(AstroItems.Collectible.AMPLIFYING_MIND, "증폭하는 마음", "...", "1초 마다 공격력이 0.005 증가합니다.#피격 시 60초 동안 증가 효과가 중단됩니다.#중첩 시 다음 증가량부터 적용됩니다.")
end
...
AstroItems:AddCallback(
    ...
    function(_, entity, amount, damageFlags, source, countdownFrames)
        ...
        if player ~= nil then
            if player:HasCollectible(AstroItems.Collectible.AMPLIFYING_MIND) then
                if damageFlags & (DamageFlag.DAMAGE_NO_PENALTIES | DamageFlag.DAMAGE_RED_HEARTS) == 0 then
                    ...
                    data.AmplifyingMindCooldown = Game():GetFrameCount() + 1 * AstroItems.MINUTE
                end
            end
        end
    end
...
local function ComputeMultiplier(player)
    local result = player:GetCollectibleNum(AstroItems.Collectible.AMPLIFYING_MIND)

    if player:GetPlayerType() == AstroItems.Players.LEAH then
