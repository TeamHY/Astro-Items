        ...
        if player ~= nil then
            ...
            if (data["superNovaCooldown"] == nil or data["superNovaCooldown"] < Game():GetFrameCount()) and player:HasCollectible(AstroItems.Collectible.SUPER_NOVA) and entity:IsVulnerableEnemy() then
                ...
                if source.Type == EntityType.ENTITY_TEAR or damageFlags & DamageFlag.DAMAGE_LASER == DamageFlag.DAMAGE_LASER or source.Type == EntityType.ENTITY_KNIFE then
                    ...
                    local baseChance = SPAWN_CHANCE * player:GetCollectibleNum(AstroItems.Collectible.SUPER_NOVA)
                    ...
                    if rng:RandomFloat() < baseChance + player.Luck * LUCK_MULTIPLY then
                        ...
                        data["superNovaCooldown"] = Game():GetFrameCount() + COOLDOWN_TIME / player:GetCollectibleNum(AstroItems.Collectible.SUPER_NOVA)
                    end
                end
            end
        end
