AstroItems.Collectible.ORIGINAL_SINFUL_SPOILS_SNAKE_EYE = Isaac.GetItemIdByName("Original Sinful Spoils - Snake Eye")

if EID then
    AstroItems:AddEIDCollectible(AstroItems.Collectible.ORIGINAL_SINFUL_SPOILS_SNAKE_EYE, "원죄보 - 스네이크아이", "...", "공격 시 10%의 확률로 여러 유령을 소환합니다.#중첩 시 기본 확률이 합 연산으로 증가하고 쿨타임이 줄어듭니다.#!!! {{LuckSmall}}행운 수치 비례: 행운 90 이상일 때 100% 확률 (행운 1당 +1%p)")
end
...
local function ComputeMultiplier(player)
    local result = player:GetCollectibleNum(AstroItems.Collectible.ORIGINAL_SINFUL_SPOILS_SNAKE_EYE)
...
AstroItems:AddCallback(
    ...
    function(_, entity, amount, damageFlags, source, countdownFrames)
        ...
        if player ~= nil then
            ...
            if (data["ossSnakeEyeCooldown"] == nil or data["ossSnakeEyeCooldown"] < Game():GetFrameCount()) and player:HasCollectible(AstroItems.Collectible.ORIGINAL_SINFUL_SPOILS_SNAKE_EYE) and entity:IsVulnerableEnemy() then
