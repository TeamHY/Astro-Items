AstroItems.Collectible.PTOLEMAEUS = Isaac.GetItemIdByName("Ptolemaeus")
...
if EID then
    AstroItems:AddEIDCollectible(AstroItems.Collectible.PTOLEMAEUS, "프톨레마이오스", "...", "발사한 눈물 주변을 회전하는 공격력 절반의 눈물이 생깁니다.")
end
...
AstroItems:AddCallback(
    ...
    function(_, tear)
        ...
        if player ~= nil and player:HasCollectible(AstroItems.Collectible.PTOLEMAEUS) then
            tearData.Ptolemaeus = {}
        end
    end
)
        ...
        if player ~= nil and tearData.Ptolemaeus ~= nil then
            if not tearData.Ptolemaeus.IsSub then -- 메인 눈물 업데이트 (서브 눈물 위치 계산)
                if tear.FrameCount == 1 then
                    ...
                    for _ = 1, player:GetCollectibleNum(AstroItems.Collectible.PTOLEMAEUS) do
                        local subTear =
