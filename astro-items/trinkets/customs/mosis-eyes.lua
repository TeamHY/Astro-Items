...
AstroItems.Trinket.MOSIS_EYES = Isaac.GetTrinketIdByName("Mosi's Eyes")

if EID then
    EID:addTrinket(AstroItems.Trinket.MOSIS_EYES, "↓ {{LuckSmall}}행운 -5#캐릭터가 있는 방에서 2칸 이내에 있는 스테이지 구조를 맵에 표시합니다.#가려진 아이템을 알 수 있게 됩니다.", "모시의 눈")
...
local function DisplayNeighborRoom()
    ...
    local currentRoomPosition = AstroItems:ConvertRoomIndexToPosition(level:GetCurrentRoomIndex())

    for x = -2, 2 do
        for y = -2, 2 do
            local roomIndex = AstroItems:ConvertRoomPositionToIndex(currentRoomPosition + Vector(x, y))
...
AstroItems:AddCallback(
    ...
    function(_)
        for i = 1, Game():GetNumPlayers() do
            ...
            if not player:HasTrinket(AstroItems.Trinket.MOSIS_EYES) then
                break
            end
...
AstroItems:AddCallback(
    ...
    function(_)
        if AstroItems:HasTrinket(AstroItems.Trinket.MOSIS_EYES) then
            TryRemoveBlind()
        end
    end
)
...
AstroItems:AddCallback(
    ...
    function(_, player, cacheFlag)
        if player:HasTrinket(AstroItems.Trinket.MOSIS_EYES) then
...
AstroItems:AddCallbackCustom(
    ...
    function(_, player, pickingUpItem)
        if pickingUpItem.itemType == ItemType.ITEM_TRINKET and pickingUpItem.subType == AstroItems.Trinket.MOSIS_EYES then
            DisplayNeighborRoom()
            TryRemoveBlind()
        end
    end
