...
AstroItems.Trinket.NOSTALGIA = Isaac.GetTrinketIdByName("Nostalgia")

if EID then
    EID:addTrinket(AstroItems.Trinket.NOSTALGIA, "#소지한 상태에서 {{BossRoom}}보스방 클리어 시 The Binding of Isaac: Wrath of the Lamb에서 등장한 아이템 2개를 소환합니다.#!!! 획득 시 바로 흡수됩니다.#!!! 효과가 발동한 뒤 사라집니다.", "노스텔지아")
end
...
AstroItems:AddCallback(
    ...
    function(_, player)
        ...
        if AstroItems:CheckTrinket(trinket0, AstroItems.Trinket.NOSTALGIA) then
            player:TryRemoveTrinket(trinket0)
            isc:smeltTrinket(player, AstroItems.Trinket.NOSTALGIA)
        elseif AstroItems:CheckTrinket(trinket1, AstroItems.Trinket.NOSTALGIA) then
...
AstroItems:AddCallback(
    ...
    function()
        ...
        for i = 1, Game():GetNumPlayers() do
            ...
            if player:HasTrinket(AstroItems.Trinket.NOSTALGIA) and currentRoom:GetType() == RoomType.ROOM_BOSS then
                local rng = player:GetTrinketRNG(AstroItems.Trinket.NOSTALGIA)

                local items = AstroItems:GetRandomCollectibles(wolItemList, rng, 2)

                for _, value in ipairs(items) do
                    AstroItems:SpawnCollectible(value, player.Position, AstroItems.Trinket.NOSTALGIA)
                end

                player:TryRemoveTrinket(AstroItems.Trinket.NOSTALGIA)
            end
        end
    end
)
